...
class LTA_Applicants_Plugin {
    public function __construct() {
        ...
        // Recruitment title resolver
        add_action('wp_ajax_lta_get_recruitment_title', array($this, 'ajax_get_recruitment_title'));
        add_action('wp_ajax_nopriv_lta_get_recruitment_title', array($this, 'ajax_get_recruitment_title'));

        // Upload CV file to WordPress media
        add_action('wp_ajax_lta_upload_cv', array($this, 'ajax_upload_cv'));
        add_action('wp_ajax_nopriv_lta_upload_cv', array($this, 'ajax_upload_cv'));
    ...
    private function create_table() {
        ...
        // Ensure applicants table exists (do not drop existing data)
        ...
        $applicants_sql = "CREATE TABLE IF NOT EXISTS $applicants_table (
            ...
            notes longtext NULL,
            recruitment_title varchar(255) NULL,
        ...
        // Backfill: add recruitment_title column if missing
        $has_col = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = %s AND COLUMN_NAME = 'recruitment_title'", $applicants_table));
        if (!$has_col) {
            $wpdb->query("ALTER TABLE $applicants_table ADD COLUMN recruitment_title varchar(255) NULL AFTER notes");
        }
    }
    ...
    public function ajax_submit_applicant() {
        ...
        $record = array(
            ...
            'recruitment_title' => sanitize_text_field($src['recruitmentTitle'] ?? ''),
    ...
    // Resolve recruitment title by ID from custom table wp_lta_recruitment
    public function ajax_get_recruitment_title() {
        if (!$this->verify_api_key()) {
            wp_die(json_encode(['success' => false, 'message' => 'API key không hợp lệ']));
        }
        $id = isset($_REQUEST['id']) ? sanitize_text_field($_REQUEST['id']) : '';
        if (empty($id)) {
            wp_die(json_encode(['success' => false, 'message' => 'Thiếu id']));
        }
        global $wpdb;
        $table = $wpdb->prefix . 'lta_recruitment';
        $row = $wpdb->get_row($wpdb->prepare("SELECT id, title, position FROM $table WHERE id = %s", $id), ARRAY_A);
        if (!$row) {
            wp_die(json_encode(['success' => false, 'message' => 'Không tìm thấy']));
        }
        wp_die(json_encode(['success' => true, 'data' => $row]));
    }

    // Handle CV upload and return public URL
    public function ajax_upload_cv() {
        if (!$this->verify_api_key()) {
            wp_die(json_encode(['success' => false, 'message' => 'API key không hợp lệ']));
        }
        if (!isset($_FILES['cv'])) {
            wp_die(json_encode(['success' => false, 'message' => 'Thiếu file CV']));
        }
        require_once(ABSPATH . 'wp-admin/includes/file.php');
        require_once(ABSPATH . 'wp-admin/includes/media.php');
        require_once(ABSPATH . 'wp-admin/includes/image.php');
        $overrides = array('test_form' => false);
        $file = wp_handle_upload($_FILES['cv'], $overrides);
        if (isset($file['error'])) {
            wp_die(json_encode(['success' => false, 'message' => $file['error']]));
        }
        // Optionally create attachment; for now, just return URL
        wp_die(json_encode(['success' => true, 'url' => $file['url']]));
    }
