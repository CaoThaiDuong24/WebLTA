...
export async function POST(req: NextRequest) {
  try {
    ...
    const resp = await fetch(`${siteUrl}/wp-json/lta/v1/categories/prune`, {
      method: 'POST',
    ...
    if (!resp.ok) {
      return NextResponse.json({ error: 'Prune thất bại', details: data }, { status: resp.status })
    }
    return NextResponse.json({ success: true, result: data })
  } catch (e: any) {
    return NextResponse.json({ error: e?.message || 'Lỗi prune' }, { status: 500 })
  }
}
